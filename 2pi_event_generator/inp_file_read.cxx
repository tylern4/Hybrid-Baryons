...
void inp_file_read(Float_t &E_beam) {
  ...
  switch (flag_bos) {
    case 0:
      ...
      break;
    case 1:
      cout << "Ouput BOS flag  " << flag_bos << "  -  output with MCTK, MCVX banks"
           << "\n";
      break;
    case 2:
  ...
  if (!(flag_bos == 0)) {
    ...
    cout << "Ouput BOS flag  " << flag_bos
         << "  - no BOS output, since 'make nobos' used (for BOS output compile 'make bos') "
         << "\n";
  }
#endif
  ...
  switch (flag_radmod) {
    case 0:
      ...
      break;
    case 1:
      cout << "Radiative mode flag  " << flag_radmod << "  -  rad eff with no straggling"
           << "\n";
      break;
    case 2:
      cout << "Radiative mode flag  " << flag_radmod << "  -  rad eff with straggling"
           << "\n";
      break;
  }
  ...
  switch (flag_flux) {
    case 0:
      cout << "Flux flag " << flag_flux << "  - under influence of virtual photons (model cross section)"
           << "\n";
      break;
    case 1:
      cout << "Flux flag " << flag_flux << "  -  under influence of electrons (like exp data)"
           << "\n";
      break;
  }
  ...
  if ((!(flag_radmod == 0)) && (flag_fermi == 1))
    cout << "\nCAUTION! Fermi mode flag = " << flag_fermi << " and Rad mode flag = " << flag_radmod
         << ".\nIn this EG these two modes are combined naively. It is recommended to use separate TWOPEG-D version, "
            "where Fermi mode and Rad mode are combined properly.\n";
  // It is not recommended to combine these two modes in this EG. If you want to combine Fermi mode with Rad mode
  // properly, please, use TWOPEG-D version. See TWOPEG-CLAS-NOTE for detailes.
  cout << "\n";
  cout << "----------------------------------------\n";
  cout << "\n";
}
