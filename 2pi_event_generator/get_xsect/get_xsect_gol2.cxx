...
// 1 - using auxiliary functions (getWbin_GOL, getsbin_GOL, getanglebin_GOL) we identify the left and right point for
// each variable generated  (according to Golovach binning) 2 - then we are doing 4d-interpolation for each Wleft_bin
// and Wright_bin and obtain sigma_wright_gol and sigma_wleft_gol

void get_xsect_gol2(Float_t Wgen, Float_t s12gen, Float_t s23gen, Float_t thetagen, Float_t alphagen,
                    Short_t &Wleft_bin, Float_t &sigma_wright_gol, Float_t &sigma_wleft_gol) {
  // using auxiliary functions (getWbin_GOL, getsbin_GOL, getanglebin_GOL) we identify the left and right point for each
  // variable generated  (according to Golovach binning)
  ...
  // S23_ARR_GOL[s23left_wleft_bin][Wleft_bin] << " "<< S23_ARR_GOL[s23right_wleft_bin][Wleft_bin] << "\n";

  // then we are doing 4d-interpolation for each Wleft_bin and Wright_bin and obtain sigma_wright_gol and
  // sigma_wleft_gol
  interpol_gol2(Wright_bin, s12left_wright_bin, s12right_wright_bin, s23left_wright_bin, s23right_wright_bin,
                thetaleft_bin, thetaright_bin, alphaleft_bin, alpharight_bin, s12gen, s23gen, thetagen, alphagen,
                sigma_wright_gol);

  interpol_gol2(Wleft_bin, s12left_wleft_bin, s12right_wleft_bin, s23left_wleft_bin, s23right_wleft_bin, thetaleft_bin,
                thetaright_bin, alphaleft_bin, alpharight_bin, s12gen, s23gen, thetagen, alphagen, sigma_wleft_gol);

  // cout <<Wgen<<" "<< sigma_wright_gol << " "<<sigma_wleft_gol<<"\n";

  // cout << Wgen << " "<< s12left_wright_bin << " "<<s12right_wright_bin<<"
  // "<<S12_ARR_GOL[s12right_wright_bin][Wright_bin] << "  "<<s23left_wright_bin<< " "<< s23right_wright_bin<< " "<<
  // sigma_wright_gol<<"\n"; cout << Wgen << " "<< s12left_wleft_bin << " "<<s12right_wleft_bin<<"
  // "<<S12_ARR_GOL[s12right_wleft_bin][Wleft_bin]<< "  "<<s23left_wleft_bin<< " "<< s23right_wleft_bin<< " "<<
  // sigma_wleft_gol<<"\n";
