...
void get_xsect_q2_130_w_gt_18_lt_21(Float_t E_beam, Float_t Q2gen, Float_t Wgen, Float_t s12gen, Float_t s23gen,
  ...
  for (Short_t i = 0; i < 6; i++) {
    interpol_rip2(4, Wright_bin, s12left_wright_bin, s12right_wright_bin, s23left_wright_bin, s23right_wright_bin,
                  thetaleft_bin, thetaright_bin, alphaleft_bin, alpharight_bin, s12gen, s23gen, thetagen, alphagen,
                  sigma_wright[i], i);

    interpol_rip2(4, Wleft_bin, s12left_wleft_bin, s12right_wleft_bin, s23left_wleft_bin, s23right_wleft_bin,
                  thetaleft_bin, thetaright_bin, alphaleft_bin, alpharight_bin, s12gen, s23gen, thetagen, alphagen,
                  sigma_wleft[i], i);
  }
  ...
  // if ((isnan(sigma_wright[0]))||(isnan(sigma_wleft[0]))) cout << sigma_wright[0]<<" "<<sigma_wleft[0]<<" "<<s12gen<<"
  // "<<S12_ARR_RIP2[][Wleft_bin] <<s23gen<<" "<<s12left_wright_bin<<" "<< s23left_wright_bin<<" "<<s12left_wleft_bin<<"
  // "<<s23left_wleft_bin<<" subr\n";

  if ((Q2gen > 0.0002) && (Q2gen <= 0.65)) {
    // 1dim W-interpolation for golovach
    sigma_t_gol = 1. / fabs(W_ARR_RIP2[Wright_bin] - W_ARR_RIP2[Wleft_bin]);
    sigma_t_gol = sigma_t_gol * (sigma_t_wright_gol * fabs(W_ARR_RIP2[Wleft_bin] - Wgen) +
    ...
    sigma_final[0] = sigma_final[0] * (sigma_wright[0] * fabs(W_ARR_RIP2[Wleft_bin] - Wgen) +
    ...
    // if ((Q2gen > 0.3)&&(Q2gen <=0.65)&&(E_beam>2.45)&&(E_beam<3.)) sigma_t_final
  ...
  if ((Q2gen >= 0.95) && (Q2gen <= 1.3)) {
    ...
    sigma_l_final = sigma_l_final * (sigma_wright[1] * fabs(W_ARR_RIP2[Wleft_bin] - Wgen) +
  ...
  for (Short_t i = 2; i < 6; i++) {
    ...
    sigma_final[i] = sigma_final[i] * (sigma_wright[i] * fabs(W_ARR_RIP2[Wleft_bin] - Wgen) +
