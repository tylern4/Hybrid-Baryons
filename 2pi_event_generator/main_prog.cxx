...
Float_t G_BYCKLING(Float_t x, Float_t y, Float_t z, Float_t u, Float_t v,
                   Float_t w) {
...
int main(int argc, char **argv) {
  ...
  Float_t sigma_t_final_1, sigma_l_final_1, sigma_c2f_final_1,
      sigma_s2f_final_1, sigma_cf_final_1, sigma_sf_final_1;
  ...
  Float_t E_beam, E_beam_fermi, theta_rot2, E_E_prime_ferm, Theta_e_prime_ferm,
      W_ferm;
  ...
  Float_t M1, M2, M3;
  Float_t inv_m12, inv_m23, inv_m13, th_hadr, alph_hadr, ph_hadr, s12, s23, s13,
      en1, en2, en3, mag1, mag2, mag3;
  Float_t inv_m12_tst, inv_m23_tst, inv_m13_tst, th_hadr_tst, alph_hadr_tst,
      ph_hadr_tst;
  ...
  if (argc > 1) {
    ...
    auto cli =
        (clipp::option("-h", "--help").set(print_help) % "print help",
         clipp::value("Nevents", Nevents),
         clipp::option("-E_beam") &
             clipp::value("E_beam", E_beam) % "Beam En. (GeV)\t[10.6]",
         clipp::option("-W_min") &
             clipp::value("W_min", W_min) % "W min (GeV)\t\t[1.4]",
         clipp::option("-W_max") &
             clipp::value("W_max", W_max) % "W max (GeV)\t\t[2.3]",
         clipp::option("-Q2_min") &
             clipp::value("Q2_min", Q2_min) % "Q2 min (GeV^2)\t[0.05]",
         clipp::option("-Q2_max") &
             clipp::value("Q2_max", Q2_max) % "Q2 max (GeV^2)\t[3.5]",
         clipp::option("-Theta_min") &
             clipp::value("Theta_min", Theta_min) %
                 "min theta scattered electron (deg)\t[1.0]",
         clipp::option("-Theta_max") &
             clipp::value("Theta_max", Theta_max) %
                 "max theta scattered electron (deg)\t[50.0]",
         clipp::option("-E_eprime_min") &
             clipp::value("E_eprime_min", E_eprime_min) %
                 "min E scattered electron (GeV)\t[0.01]",
         ...
         clipp::option("-Targ_radlen") &
             clipp::value("Targ_radlen", Targ_radlen),
         clipp::option("-Targ_len") & clipp::value("Targ_len", Targ_len),
         ...
         clipp::option("-Twi_Z") & clipp::value("Twi_Z", Twi_Z),
         clipp::option("-Twf_Z") & clipp::value("Twf_Z", Twf_Z),
         clipp::option("-Twi_A") & clipp::value("Twi_A", Twf_dens),
         clipp::option("-Twf_A") & clipp::value("Twf_A", Twf_dens),
         clipp::option("-flag_radmod") &
             clipp::value("flag_radmod", flag_radmod),
  ...
  Q2lim1 = 2 * E_beam * sin(Theta_min * M_PI / 180. / 2.) *
           sin(Theta_min * M_PI / 180. / 2.);
  Q2lim1 = Q2lim1 * (2 * E_beam * MP - W_max * W_max + MP * MP);
  Q2lim1 = Q2lim1 / (MP + 2 * E_beam * sin(Theta_min * M_PI / 180. / 2.) *
                              sin(Theta_min * M_PI / 180. / 2.));
  ...
  Q2lim2 = 2 * E_beam * sin(Theta_max * M_PI / 180. / 2.) *
           sin(Theta_max * M_PI / 180. / 2.);
  Q2lim2 = Q2lim2 * (2 * E_beam * MP - W_min * W_min + MP * MP);
  Q2lim2 = Q2lim2 / (MP + 2 * E_beam * sin(Theta_max * M_PI / 180. / 2.) *
                              sin(Theta_max * M_PI / 180. / 2.));
  ...
  TRandom3 th_hadr_rndm(
      UInt_t(((float)rand() / (float)(RAND_MAX)) * 4000000000.));
  ...
  TRandom3 ph_hadr_rndm(
      UInt_t(((float)rand() / (float)(RAND_MAX)) * 4000000000.));
  TRandom3 r_vert_rndm(
      UInt_t(((float)rand() / (float)(RAND_MAX)) * 4000000000.));
  TRandom3 phi_vert_rndm(
      UInt_t(((float)rand() / (float)(RAND_MAX)) * 4000000000.));
  // This is for Fermi motion only
  TRandom3 fermi_R_rndm(
      UInt_t(((float)rand() / (float)(RAND_MAX)) * 4000000000.));
  TRandom3 fermi_R1_rndm(
      UInt_t(((float)rand() / (float)(RAND_MAX)) * 4000000000.));
  TRandom3 fermi_theta_rndm(
      UInt_t(((float)rand() / (float)(RAND_MAX)) * 4000000000.));
  TRandom3 fermi_phi_rndm(
      UInt_t(((float)rand() / (float)(RAND_MAX)) * 4000000000.));
  // This is for radeff
  TRandom3 phot_rndm(UInt_t(((float)rand() / (float)(RAND_MAX)) * 4000000000.));
  TRandom3 hardini_rndm(
      UInt_t(((float)rand() / (float)(RAND_MAX)) * 4000000000.));
  TRandom3 hardfin_rndm(
      UInt_t(((float)rand() / (float)(RAND_MAX)) * 4000000000.));
  ...
  for (Int_t i = 1; i <= Nevents; i++) {
    ...
    z_EL =
        z_EL_rndm.Uniform(Targ_off - Targ_len / 2., Targ_off + Targ_len / 2.);
    ...
    do {
      k++;
      if ((k % 1000) == 0)
        cout << "N generated = " << k << " N accepted = " << i << "\r\r"
             << std::flush;
    ...
    P4_E_prime.SetXYZT(E_E_prime * cos(phi_e) * sin(Theta_e_prime),
                       E_E_prime * sin(phi_e) * sin(Theta_e_prime),
    ...
    if ((flag_radmod == 1) || (flag_radmod == 2)) {
      radcorr(phot_rndm.Uniform(0., 1.), hardini_rndm.Uniform(0., 1.),
              hardfin_rndm.Uniform(0., 1.), E_beam, Q2, W, Wnew, Q2new,
              E_beam_new, e_rad_phot, cr_rad_fact);
    ...
    P4_E_prime_new.SetXYZT(E_E_prime_new * cos(phi_e) * sin(Theta_e_prime),
                           E_E_prime_new * sin(phi_e) * sin(Theta_e_prime),
                           E_E_prime_new * cos(Theta_e_prime), E_E_prime_new);
    ...
    if (flag_fermi == 1) {
      do {
        fermi_bonn(fermi_R_rndm.Uniform(0., 0.9999679),
                   fermi_R1_rndm.Uniform(0., 1.),
                   fermi_theta_rndm.Uniform(-1., 1.),
                   fermi_phi_rndm.Uniform(0., 2. * M_PI));
      ...
      // for fermi motion
      fermi_rot(E_beam_fermi, theta_rot2, E_beam_new, P4_E_prime_new,
                P4_E_prime_boosted);
    ...
    if ((W >= 1.4125) && (W <= 1.8125) && (Q2 >= 0.65) && (Q2 <= 1.3)) {
      get_xsect_ripani(Q2, W, s12, s23, th_hadr, alph_hadr, ph_hadr,
                       sigma_t_final, sigma_l_final, sigma_c2f_final,
                       sigma_s2f_final, sigma_cf_final, sigma_sf_final);
    }

    if ((W >= 1.6125) && (W <= 1.8125) && (Q2 > 0.000001) && (Q2 < 0.65)) {
      get_xsect_14_18_lowq2_fit(Q2, W, s12, s23, th_hadr, alph_hadr, ph_hadr,
                                sigma_t_final, sigma_l_final, sigma_c2f_final,
                                sigma_s2f_final, sigma_cf_final,
                                sigma_sf_final);
    }
    ...
    if (((W >= 1.3125) && (W <= 1.4375) && (Q2 > 0.275) && (Q2 < 0.575)) ||
      ...
      get_xsect_fedotov(Q2, W, s12, s23, th_hadr, alph_hadr, ph_hadr,
                        sigma_t_final, sigma_l_final, sigma_c2f_final,
                        sigma_s2f_final, sigma_cf_final, sigma_sf_final);
    }
    ...
    if (((W >= 1.4125) && (W <= 1.4375) && (Q2 >= 0.575) && (Q2 <= 0.65)) ||
      ...
      get_xsect_rip_fed_join(Q2, W, s12, s23, th_hadr, alph_hadr, ph_hadr,
                             sigma_t_final, sigma_l_final, sigma_c2f_final,
                             sigma_s2f_final, sigma_cf_final, sigma_sf_final);
    }

    if ((W >= 1.2375) && (W < 1.3125) && (Q2 > 0.00002) && (Q2 < 1.3)) {
      get_xsect_near_threshold(Q2, W, s12, s23, th_hadr, alph_hadr, ph_hadr,
                               sigma_t_final, sigma_l_final, sigma_c2f_final,
                               sigma_s2f_final, sigma_cf_final, sigma_sf_final);
    }

    if ((W >= 1.8375) && (W <= 2.5375) && (Q2 > 0.00002) && (Q2 < 1.3)) {
      get_xsect_q2_130_w_gt_18_lt_21(
          E_beam, Q2, W, s12, s23, th_hadr, alph_hadr, ph_hadr, sigma_t_final,
          sigma_l_final, sigma_c2f_final, sigma_s2f_final, sigma_cf_final,
          sigma_sf_final);
    }

    if ((W >= 2.5875) && (W <= 3.0375) && (Q2 > 0.00002) && (Q2 < 1.3)) {
      get_xsect_25_30(Q2, W, s12, s23, th_hadr, alph_hadr, ph_hadr,
                      sigma_t_final, sigma_l_final, sigma_c2f_final,
                      sigma_s2f_final, sigma_cf_final, sigma_sf_final);
    }

    if ((W >= 1.5875) && (W <= 1.6125) && (Q2 > 0.00001) && (Q2 < 0.65)) {
      get_xsect_14_18_lowq2_fit(Q2, 1.6125, s12, s23, th_hadr, alph_hadr,
                                ph_hadr, sigma_t_final_2, sigma_l_final_2,
                                sigma_c2f_final_2, sigma_s2f_final_2,
                                sigma_cf_final_2, sigma_sf_final_2);

      get_xsect_rip_fed_join(Q2, 1.5875, s12, s23, th_hadr, alph_hadr, ph_hadr,
                             sigma_t_final_1, sigma_l_final_1,
                             sigma_c2f_final_1, sigma_s2f_final_1,
                             sigma_cf_final_1, sigma_sf_final_1);

      sigma_t_final = 1. / 0.025;
      sigma_t_final = sigma_t_final * (sigma_t_final_2 * fabs(1.5875 - W) +
                                       sigma_t_final_1 * fabs(1.6125 - W));

      sigma_l_final = 1. / 0.025;
      sigma_l_final = sigma_l_final * (sigma_l_final_2 * fabs(1.5875 - W) +
                                       sigma_l_final_1 * fabs(1.6125 - W));

      sigma_c2f_final = 1. / 0.025;
      sigma_c2f_final =
          sigma_c2f_final * (sigma_c2f_final_2 * fabs(1.5875 - W) +
                             sigma_c2f_final_1 * fabs(1.6125 - W));

      sigma_s2f_final = 1. / 0.025;
      sigma_s2f_final =
          sigma_s2f_final * (sigma_s2f_final_2 * fabs(1.5875 - W) +
                             sigma_s2f_final_1 * fabs(1.6125 - W));

      sigma_cf_final = 1. / 0.025;
      sigma_cf_final = sigma_cf_final * (sigma_cf_final_2 * fabs(1.5875 - W) +
                                         sigma_cf_final_1 * fabs(1.6125 - W));

      sigma_sf_final = 1. / 0.025;
      sigma_sf_final = sigma_sf_final * (sigma_sf_final_2 * fabs(1.5875 - W) +
                                         sigma_sf_final_1 * fabs(1.6125 - W));
    }

    if ((W >= 1.8125) && (W <= 1.8375) && (Q2 > 0.00002) && (Q2 <= 1.3)) {
      if ((Q2 > 0.00002) && (Q2 <= 0.65))
        get_xsect_14_18_lowq2_fit(Q2, 1.8125, s12, s23, th_hadr, alph_hadr,
                                  ph_hadr, sigma_t_final_2, sigma_l_final_2,
                                  sigma_c2f_final_2, sigma_s2f_final_2,
                                  sigma_cf_final_2, sigma_sf_final_2);

      if ((Q2 > 0.65) && (Q2 <= 1.3))
        get_xsect_ripani(Q2, 1.8125, s12, s23, th_hadr, alph_hadr, ph_hadr,
                         sigma_t_final_2, sigma_l_final_2, sigma_c2f_final_2,
                         sigma_s2f_final_2, sigma_cf_final_2, sigma_sf_final_2);

      get_xsect_q2_130_w_gt_18_lt_21(
          E_beam, Q2, 1.8375, s12, s23, th_hadr, alph_hadr, ph_hadr,
          sigma_t_final_1, sigma_l_final_1, sigma_c2f_final_1,
          sigma_s2f_final_1, sigma_cf_final_1, sigma_sf_final_1);

      sigma_t_final = 1. / 0.025;
      sigma_t_final = sigma_t_final * (sigma_t_final_2 * fabs(1.8375 - W) +
                                       sigma_t_final_1 * fabs(1.8125 - W));

      sigma_l_final = 1. / 0.025;
      sigma_l_final = sigma_l_final * (sigma_l_final_2 * fabs(1.8375 - W) +
                                       sigma_l_final_1 * fabs(1.8125 - W));

      sigma_c2f_final = 1. / 0.025;
      sigma_c2f_final =
          sigma_c2f_final * (sigma_c2f_final_2 * fabs(1.8375 - W) +
                             sigma_c2f_final_1 * fabs(1.8125 - W));

      sigma_s2f_final = 1. / 0.025;
      sigma_s2f_final =
          sigma_s2f_final * (sigma_s2f_final_2 * fabs(1.8375 - W) +
                             sigma_s2f_final_1 * fabs(1.8125 - W));

      sigma_cf_final = 1. / 0.025;
      sigma_cf_final = sigma_cf_final * (sigma_cf_final_2 * fabs(1.8375 - W) +
                                         sigma_cf_final_1 * fabs(1.8125 - W));

      sigma_sf_final = 1. / 0.025;
      sigma_sf_final = sigma_sf_final * (sigma_sf_final_2 * fabs(1.8375 - W) +
                                         sigma_sf_final_1 * fabs(1.8125 - W));
    }

    if ((W >= 2.5375) && (W <= 2.5875) && (Q2 > 0.00001) && (Q2 < 1.3)) {
      get_xsect_q2_130_w_gt_18_lt_21(
          E_beam, Q2, 2.5375, s12, s23, th_hadr, alph_hadr, ph_hadr,
          sigma_t_final_2, sigma_l_final_2, sigma_c2f_final_2,
          sigma_s2f_final_2, sigma_cf_final_2, sigma_sf_final_2);

      get_xsect_25_30(Q2, 2.5875, s12, s23, th_hadr, alph_hadr, ph_hadr,
                      sigma_t_final_1, sigma_l_final_1, sigma_c2f_final_1,
                      sigma_s2f_final_1, sigma_cf_final_1, sigma_sf_final_1);

      sigma_t_final = 1. / 0.05;
      sigma_t_final = sigma_t_final * (sigma_t_final_2 * fabs(2.5875 - W) +
                                       sigma_t_final_1 * fabs(2.5375 - W));

      sigma_l_final = 1. / 0.05;
      sigma_l_final = sigma_l_final * (sigma_l_final_2 * fabs(2.5875 - W) +
                                       sigma_l_final_1 * fabs(2.5375 - W));

      sigma_s2f_final = 1. / 0.05;
      sigma_s2f_final =
          sigma_s2f_final * (sigma_s2f_final_2 * fabs(2.5875 - W) +
                             sigma_s2f_final_1 * fabs(2.5375 - W));

      sigma_c2f_final = 1. / 0.05;
      sigma_c2f_final =
          sigma_c2f_final * (sigma_c2f_final_2 * fabs(2.5875 - W) +
                             sigma_c2f_final_1 * fabs(2.5375 - W));

      sigma_cf_final = 1. / 0.05;
      sigma_cf_final = sigma_cf_final * (sigma_cf_final_2 * fabs(2.5875 - W) +
                                         sigma_cf_final_1 * fabs(2.5375 - W));

      sigma_sf_final = 1. / 0.05;
      sigma_sf_final = sigma_sf_final * (sigma_sf_final_2 * fabs(2.5875 - W) +
                                         sigma_sf_final_1 * fabs(2.5375 - W));
    }

    if ((W >= 3.1375) && (W <= 4.5375) && (Q2 > 0.00001) && (Q2 < 1.3)) {
      get_xsect_q2_13_wgt_3(Q2, W, s12, s23, th_hadr, alph_hadr, ph_hadr,
                            sigma_t_final, sigma_l_final, sigma_c2f_final,
                            sigma_s2f_final, sigma_cf_final, sigma_sf_final);
    }

    if ((W >= 3.0375) && (W <= 3.1375) && (Q2 > 0.00001) && (Q2 < 1.3)) {
      get_xsect_25_30(Q2, 3.0375, s12, s23, th_hadr, alph_hadr, ph_hadr,
                      sigma_t_final_2, sigma_l_final_2, sigma_c2f_final_2,
                      sigma_s2f_final_2, sigma_cf_final_2, sigma_sf_final_2);

      get_xsect_q2_13_wgt_3(Q2, 3.1375, s12, s23, th_hadr, alph_hadr, ph_hadr,
                            sigma_t_final_1, sigma_l_final_1, sigma_c2f_final_1,
                            sigma_s2f_final_1, sigma_cf_final_1,
                            sigma_sf_final_1);

      sigma_t_final = 1. / 0.1;
      sigma_t_final = sigma_t_final * (sigma_t_final_2 * fabs(3.1375 - W) +
                                       sigma_t_final_1 * fabs(3.0375 - W));

      sigma_l_final = 1. / 0.1;
      sigma_l_final = sigma_l_final * (sigma_l_final_2 * fabs(3.1375 - W) +
                                       sigma_l_final_1 * fabs(3.0375 - W));

      sigma_c2f_final = 1. / 0.1;
      sigma_c2f_final =
          sigma_c2f_final * (sigma_c2f_final_2 * fabs(3.1375 - W) +
                             sigma_c2f_final_1 * fabs(3.0375 - W));

      sigma_s2f_final = 1. / 0.1;
      sigma_s2f_final =
          sigma_s2f_final * (sigma_s2f_final_2 * fabs(3.1375 - W) +
                             sigma_s2f_final_1 * fabs(3.0375 - W));

      sigma_cf_final = 1. / 0.1;
      sigma_cf_final = sigma_cf_final * (sigma_cf_final_2 * fabs(3.1375 - W) +
                                         sigma_cf_final_1 * fabs(3.0375 - W));

      sigma_sf_final = 1. / 0.1;
      sigma_sf_final = sigma_sf_final * (sigma_sf_final_2 * fabs(3.1375 - W) +
                                         sigma_sf_final_1 * fabs(3.0375 - W));
    }

    // Extending to the region Q2 > 1.3 GeV2
    if (Q2nodata > 1.299) {
      sigma_t_final =
          sigma_t_final * Func_q2_dep(Q2nodata) / Func_q2_dep(1.299);
      sigma_l_final =
          sigma_l_final * Func_q2_dep(Q2nodata) / Func_q2_dep(1.299);
      sigma_c2f_final =
          sigma_c2f_final * Func_q2_dep(Q2nodata) / Func_q2_dep(1.299);
      sigma_s2f_final =
          sigma_s2f_final * Func_q2_dep(Q2nodata) / Func_q2_dep(1.299);
      sigma_cf_final =
          sigma_cf_final * Func_q2_dep(Q2nodata) / Func_q2_dep(1.299);
      sigma_sf_final =
          sigma_sf_final * Func_q2_dep(Q2nodata) / Func_q2_dep(1.299);
    }
    ...
    eps_t = 1. / (1. + 2. * (1. + nu_g * nu_g / Q2) * tan(theta_el / 2.) *
                           tan(theta_el / 2.));
    ...
    sigma_total = sigma_total + eps_t * (sigma_c2f_final * cos(2. * ph_hadr) +
                                         sigma_s2f_final * sin(2. * ph_hadr));
    sigma_total = sigma_total + sqrt(2. * eps_l * (eps_t + 1)) *
    ...
    if ((flag_flux == 1) && (flag_fermi == 0)) {
      ...
      eps_t = 1. / (1. + 2. * (1. + nu_g * nu_g / Q2_old) * tan(theta_el / 2.) *
                             tan(theta_el / 2.));
    ...
    anti_rot(W, Q2, phi_e, E_beam_fermi, inv_m12, inv_m23, th_hadr, alph_hadr,
             ph_hadr, MPIM, MPIP, MP, P4_PIM, P4_PIP, P4_Pfin);
    ...
    // Second set of varibles. FOR 1-PIM, 2-PIP, 3-P
    if (flag_fermi == 0)
      rot(Q2, E_beam_fermi, P4_E_prime_new, P4_PIM, P4_PIP, P4_Pfin,
          inv_m12_tst, inv_m23_tst, th_hadr_tst, alph_hadr_tst, ph_hadr_tst);
    if (flag_fermi == 1)
      rot(Q2, E_beam_fermi, P4_E_prime_boosted, P4_PIM, P4_PIP, P4_Pfin,
          inv_m12_tst, inv_m23_tst, th_hadr_tst, alph_hadr_tst, ph_hadr_tst);
    ...
    if (flag_fermi == 1)
      fermi_anti_rot(W, Q2, E_beam_new, E_beam_fermi, phi_e, theta_rot2, P4_PIM,
                     P4_PIP, P4_Pfin, P4_E_prime_new);
    ...
    out_file_fill(i, sigma_total, W, Q2, P4_E_prime, P4_Pfin, P4_PIP, P4_PIM,
                  z_EL, x_EL, y_EL);

    // filling out histograms
    hist_fill(E_beam, W, W_old, Q2, Q2_old, phi_e, z_EL, s12, s23, th_hadr,
              alph_hadr, ph_hadr, sigma_t_final, sigma_l_final, eps_l,
