...
int main(int argc, char **argv) {
  ...
  /*
    std::string PDGDATA;
    if (getenv("ROOTPDG") != NULL) {
      PDGDATA = string(getenv("ROOTPDG")) + "/pdg_table.txt";
    } else if (getenv("ROOTSYS") != NULL) {
      PDGDATA = string(getenv("ROOTSYS")) + "/etc/pdg_table.txt";
    } else {
      std::cerr << "$ROOTSYS not set" << std::endl;
      exit(1);
    }
  */
  data_dir_2pi = string(getenv("data_dir_2pi"));
  cout << "DATA DIR IS " << data_dir_2pi << endl;
  ...
  pdg->ReadPDGTable(PDGDATA.c_str());
  ...
  for (Int_t i = 1; i <= Nevents; i++) {
    ...
    do {
      ...
      Q2lim2 = Q2lim2 / (MP + 2 * E_beam * sin(Theta_max * M_PI / 180. / 2.) * sin(Theta_max * M_PI / 180. / 2.));
             ...
             (isnan(acos((Q2 + 2. * E_beam * nu) / 2. / E_beam / (sqrt(Q2 + nu * nu))))));
    ...
    do {
      ...
      dummy_int++;
             ...
             (isnan(acos((M1 * M1 + M2 * M2 + 2 * en1 * en2 - s12) / 2. / mag1 / mag2))) ||
             (isnan(acos((M1 * M1 + M3 * M3 + 2 * en1 * en3 - s13) / 2. / mag1 / mag3))) || (en1 < M1) || (en2 < M2) ||
             (en3 < M3) || (sqrt(s13) < M1 + M3) || (sqrt(s13) > W - M2));
    ...
    if (((W >= 1.4125) && (W <= 1.4375) && (Q2 >= 0.575) && (Q2 <= 0.65)) ||
      ...
      // TODO:: Keep getting segfaults here. If I add a print statement they disapear?
      // std::cout << "this " << thiss++ << "\r\r" << std::flush;
    ...
    */
    ...
    // anti_rot(W, Q2, phi_e, E_beam_fermi, inv_m12, inv_m23, th_hadr, alph_hadr,  ph_hadr, MP,MPIP, MPIM,
    // P4_Pfin,P4_PIP, P4_PIM);

    // Third set of variables. FOR 1-PIP, 2-PIM, 3-P
    // anti_rot(W, Q2, phi_e, E_beam_fermi, inv_m12, inv_m23, th_hadr, alph_hadr,  ph_hadr, MPIP,MPIM, MP,
    // P4_PIP,P4_PIM, P4_Pfin);
    ...
    // test
    // if ((fabs(th_hadr-th_hadr_tst)>0.001)||(fabs(ph_hadr-ph_hadr_tst)>0.001)||(fabs(alph_hadr-alph_hadr_tst)>0.001))
    // cout <<"ALARM!  Four-momenta of the final particles are probably WRONG!\n";
    ...
    if ((isnan(P4_E_prime[0])) || (isnan(P4_E_prime[1])) || (isnan(P4_E_prime[2])) || (isnan(P4_E_prime[3])))
      cout << P4_E_prime[0] << " " << P4_E_prime[1] << " " << P4_E_prime[2] << " " << P4_E_prime[3] << " " << W << " "
           << Q2 << " final electron is nan \n";

    if ((isnan(P4_Pfin[0])) || (isnan(P4_Pfin[1])) || (isnan(P4_Pfin[2])) || (isnan(P4_Pfin[3])))
      cout << P4_Pfin[0] << " " << P4_Pfin[1] << " " << P4_Pfin[2] << " " << P4_Pfin[3] << " " << W << " " << Q2
           << " final proton is nan \n";

    if ((isnan(P4_PIP[0])) || (isnan(P4_PIP[1])) || (isnan(P4_PIP[2])) || (isnan(P4_PIP[3])))
      cout << P4_PIP[0] << " " << P4_PIP[1] << " " << P4_PIP[2] << " " << P4_PIP[3] << " " << W << " " << Q2
           << " pip is nan \n";

    if ((isnan(P4_PIM[0])) || (isnan(P4_PIM[1])) || (isnan(P4_PIM[2])) || (isnan(P4_PIM[3])))
