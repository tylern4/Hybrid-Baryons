...
//* calculation of 2dim linearly interpolated intergated 2pi cross-section (in W-Q2 variables) including calculation
// sigma_t and sigma_l separately and their combonation with eps_L
...
Float_t s1_radsoft(Float_t E_beam, Float_t Q2gen, Float_t Wgen) {
  ...
  Delta_t = (Bwi * T_wi + 0.5 * Btarg * T_targ) * log(E_beam / delta) +
            (Bwf * T_wf + 0.5 * Btarg * T_targ) * log((E_beam - E_gamma_lab) / delta);
  Delta_t = -1. * Delta_t;

  Delta_r = 28. / 9. - (13. / 6.) * log(Q2gen / Me / Me) +
            (log(E_beam / delta) + log((E_beam - E_gamma_lab) / delta)) * (log(Q2gen / Me / Me) - 1.) -
            Spence(-E_gamma_lab / (E_beam - E_gamma_lab)) - Spence(E_gamma_lab / E_beam);
...
void radcorr(Float_t R, Double_t R_ini, Double_t R_fin, Float_t E_beam, Float_t Q2gen, Float_t Wgen, Float_t &Wnew,
             Float_t &Q2new, Float_t &E_beam_new, Float_t &e_radgam, Float_t &cr_rad_fact) {
  // Calculating lengthes of the target and i/f windows using input-file information
  Float_t sec_test_t, sec_test_l, sec_total;
  ...
  // cout << "s1 = "<< s1_radsoft(E_beam, 0.9,1.35)<< ", s2 = "<< s2_radhardini(E_beam,0.9,1.35)<< ", s3 =
  ...
  if ((R >= s1 / (s1 + s2 + s3)) && (R < (s1 + s2) / (s1 + s2 + s3))) {
    TH1F *h_radhardini =
        new TH1F("h_radhardini", "h_radhardini", 800, ARR_e_radhardini[0], ARR_e_radhardini[Npoints - 1]);
  ...
  if ((R >= (s1 + s2) / (s1 + s2 + s3)) && (R < 1.)) {
    TH1F *h_radhardfin =
        new TH1F("h_radhardfin", "h_radhardfin", 800, ARR_e_radhardfin[0], ARR_e_radhardfin[Npoints - 1]);
